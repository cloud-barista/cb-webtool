    ...
    if(!isEmpty(mcisList) && mcisList.length > 0 ){
        ...
        for(var mcisIndex in mcisList){
            ...
            try{
                ...
                if (typeof vmListOfMcis !== 'undefined' && vmListOfMcis.length > 0) {
                    for(var vmIndex in vmListOfMcis){
                        ...
                        addVm += '<div className="shot bgbox_' + vmDispStatus +'">'
                        addVm += '    <a href="javascript:void(0);"><span>' + vmIndex + '</span></a>'
                        // for map
            ...
            try{
                ...
                addMcis += '    <div class="areabox dbinfo cursor active" id="mcis_areabox_'+ mcisIndex +'" onclick="selectMcis(\'' + aMcis.id + '\',\'' + aMcis.name + '\',\'mcis_areabox_' + mcisIndex + '\', this)">'
                ...
                addMcis += '                <span class="line">/</span> <span class="num color_y">' + vmStatusCountMap.get("stop") + '</span>'
                addMcis += '                <span class="line">/</span> <span class="num color_r">' + vmStatusCountMap.get("terminate") + '</span>'
